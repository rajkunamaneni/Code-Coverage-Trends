        ...
        try:
            if pr_file_prefix in pr_csv_file:
                ...
                for idx, pop_date_commit_dict_inst in enumerate(pop_csv_dict_of_df):
                    if (pop_date_commit_dict_inst['Username'] == username) and \
                            (pop_date_commit_dict_inst['Repository'] == repository):
                        pop_csv_dict_of_df[idx]['Total Pull Requests Before Timestamp'] = \
                            get_comulative_pr_data(pop_date_commit_dict_inst['Timestamp'], pr_csv_df)
        except ValueError:
